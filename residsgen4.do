...
global skresidcorr=(abs(R[1,2])*R[1,2]*$shresidcorr+abs(R[1,3])*R[1,3]*$ndgresidcorr)/(abs(R[1,2])+abs(R[1,3]))
global yiniresidcorr=(abs(R[4,2])*R[4,2]*$shresidcorr+abs(R[4,3])*R[4,3]*$ndgresidcorr)/(abs(R[4,2])+abs(R[4,3]))
global feresidcorr=(abs(R[5,2])*R[5,2]*$shresidcorr+abs(R[5,3])*R[5,3]*$ndgresidcorr)/(abs(R[5,2])+abs(R[5,3]))
...
matrix accum XX=(lsk11 lsh12 lndg12 lyini12 fes12), nocons dev
matrix XX=XX/(r(N)-1)
matrix A=($fevar^0.5)*$fecorr*XX[1..4,5]
matrix XX[1,5]=A
matrix XX[5,1]=A'
